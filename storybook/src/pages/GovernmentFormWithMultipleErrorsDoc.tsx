...
interface GovernmentFormWithMultipleErrorsDocProps {
  isDark: boolean;
}
...
export const GovernmentFormWithMultipleErrorsDoc: React.FC<GovernmentFormWithMultipleErrorsDocProps> = ({ isDark }) => {
  const [activeMainTab, setActiveMainTab] = useState<MainTab>('preview');
  const [variant, setVariant] = useState<VariantType>('Default');

  // Form interactive states matching Flutter GovernmentFormMultipleErrorsScreen
  const [fullName] = useState('Ramesh Kumar');
  const [mobileNumber, setMobileNumber] = useState('98765');
  const [emailAddress, setEmailAddress] = useState('ramesh@example');
  const [aadhaarNumber, setAadhaarNumber] = useState('1234');
  ...
  const colors = useMemo(() => {
    ...
    return {
      ...
      errorBannerBg: isDark ? '#450A0A' : '#FFF8F8',
      errorBannerBorder: isDark ? '#7F1D1D' : '#FFDADC',
      errorBannerText: isDark ? '#FCA5A5' : '#8A1A16',
      errorColor: '#D92D20',
      errorBorder: '#EF4444',
    };
  }, [isDark, variant]);
  ...
  return (
    ...
    >
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <Ux4gStatusBanner
          ...
          subtitleWidget={
            <TouchableOpacity activeOpacity={0.7}>
              <Text style={styles.jumpText}>Jump to first error</Text>
            </TouchableOpacity>
          }
        />
        ...
        {/* SMS Notice */}
        <Text style={[styles.smsNotice, { color: titleColor }]}>
...
const styles = StyleSheet.create({
  ...
  jumpText: { color: '#8A1A16', fontSize: 14, fontWeight: '700', marginTop: 4 },
  ...
  const renderFormContent = (isCard: boolean) => {
    ...
    return (
      <>
        ...
        {/* Field 2: Mobile number (with Error) */}
        <div ref={firstErrorRef} style={{ width: '100%', marginBottom: 24 }}>
          <label
            style={{
              display: 'block',
              fontSize: 14,
          ...
          <div style={{ marginTop: 6, fontSize: 12, color: colors.errorColor, fontWeight: 500 }}>
            Mobile must be 10 digits.
          </div>
        </div>

        {/* Field 3: Email address (with Error) */}
        ...
        {/* Field 4: Aadhaar number (with Error) */}
        <div style={{ width: '100%', marginBottom: 24 }}>
          <label
            style={{
              display: 'block',
    ...
    <div className="wb-page">
      ...
      <div className="wb-header">
        <div className="wb-header-row">
          <h1 className="wb-title">Government form with multiple errors</h1>
...
export default GovernmentFormWithMultipleErrorsDoc;
