...
export const InlineFeedbackHintDoc: React.FC<InlineFeedbackHintDocProps> = ({ isDark }) => {
  ...
  // Format Aadhaar text with space every 4 digits
  const handleAadhaarChange = (text: string) => {
    ...
    setAadhaarValue(formatted);
  };

  const cleanAadhaar = aadhaarValue.replace(/\s+/g, '');
  const isComplete = cleanAadhaar.length === 12;
  const isValid = isComplete && validateAadhaar(aadhaarValue);
  ...
  const colors = useMemo(() => {
    return {
      ...
      hintColor: isDark ? UX4GColors.neutral400 : '#6B7280',
      screenBg: isCard
        ? isDark
  ...
  }, [isDark, isCard, isValid, isInvalid]);
  ...
  return (
    <SafeAreaView style={[styles.safeArea, { backgroundColor: screenBg }]}>
      ...
      <View style={styles.content}>
        ...
        {/* Aadhaar Input Field with built-in Verhoeff validation and hint */}
        <Ux4gAadhaarInputField
          value={aadhaarValue}
...
`;
  ...
  const renderFormFields = () => (
    <div>
      ...
      {/* Aadhaar Input box matching exact design */}
      <div
        style={{
          display: 'flex',
          alignItems: 'center',
      ...
      >
        ...
        {isValid && (
          <span
            className="material-symbols-outlined"
            style={{ fontSize: '18px', color: colors.successColor }}
          >
            check_circle
          </span>
        )}
        {isInvalid && (
          <span
            className="material-symbols-outlined"
            style={{ fontSize: '18px', color: colors.errorColor }}
          >
            cancel
          </span>
        )}
      ...
      >
        {isValid ? (
          <>
            <span
              className="material-symbols-outlined"
              style={{ fontSize: '16px', color: colors.successColor }}
            >
              check_circle
            </span>
            <span
          ...
          <>
            <span
              className="material-symbols-outlined"
              style={{ fontSize: '16px', color: colors.errorColor }}
            >
              error
            </span>
            <span
    ...
    <div className="wb-page">
      ...
      <div className="wb-header">
        <h1 className="wb-title">
          Inline Feedback Hint ({isCard ? 'Card Style' : 'Default'})
        </h1>
        <p className="wb-description">
          Aadhaar input with built-in Verhoeff validation. Type a 12-digit number and the field auto-validates, showing success or error feedback inline.
        </p>
      </div>
            ...
            {activeMainTab === 'code' && (
              <div className="wb-code-area">
                ...
                <CodeBlock
                  ...
                  filename={
                    variant === 'card'
                      ? 'InlineFeedbackHintCardPattern.tsx'
                      : 'InlineFeedbackHintDefaultPattern.tsx'
                  }
                />
              </div>
            )}
...
export default InlineFeedbackHintDoc;
