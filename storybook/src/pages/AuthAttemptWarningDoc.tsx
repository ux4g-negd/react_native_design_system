...
interface AuthAttemptWarningDocProps {
  isDark: boolean;
}
...
export const AuthAttemptWarningDoc: React.FC<AuthAttemptWarningDocProps> = ({ isDark }) => {
  ...
  const colors = useMemo(() => {
    return {
      ...
      bannerTitle: isDark ? UX4GColors.orange300 : UX4GColors.orange800,
      bannerSubtitle: isDark ? UX4GColors.orange300 : UX4GColors.orange800,
      bannerIcon: isDark ? UX4GColors.orange500 : UX4GColors.orange600,
      attemptPillBg: isDark ? UX4GColors.orange800 : UX4GColors.orange100,
      attemptPillText: isDark ? UX4GColors.orange300 : UX4GColors.orange800,
    };
  }, [isDark]);

  // Exact TSX Code Strings matching Flutter Widgetbook _authOtpAttemptWarningCode & _authOtpAttemptWarningCardCode
...
export const AuthOtpAttemptWarningCardPattern = ({ isDark = false }: { isDark?: boolean }) => {
    ...
    <View style={[styles.container, { backgroundColor: isDark ? UX4GColors.primary800 : UX4GColors.primary100 }]}>
      ...
      <View style={styles.cardWrapper}>
        <View style={[styles.card, { backgroundColor: isDark ? UX4GColors.neutral900 : UX4GColors.neutral50 }]}>
          ...
          {/* Ux4gStatusBanner - warningLight variant with Attempt pill */}
          <Ux4gStatusBanner
            ...
            subtitleWidget={
              <View style={styles.bannerSubtitleRow}>
                <Text style={[styles.bannerSubtitleText, { color: isDark ? UX4GColors.orange300 : UX4GColors.orange800 }]}>
                  {'1 more incorrect entry\\nbefore 30-min lockout'}
                </Text>
                <View style={[styles.attemptPill, { backgroundColor: isDark ? UX4GColors.orange800 : UX4GColors.orange100 }]}>
                  <Text style={[styles.attemptPillText, { color: isDark ? UX4GColors.orange300 : UX4GColors.orange800 }]}>
                    Attempt 1 of 3
                  </Text>
                </View>
              </View>
...
export const AuthOtpAttemptWarningDefaultPattern = ({ isDark = false }: { isDark?: boolean }) => {
  const [otp, setOtp] = useState('555555');
      ...
      <div className="wb-header">
        <div>
          <div className="wb-breadcrumb">
            <span>Patterns</span> / <span>Identity and Access</span> / <span>Auth errors and lockout</span> / <span className="active">OTP error — attempt warning</span>
          </div>
          <h1 className="wb-title">OTP error — attempt warning</h1>
          <p className="wb-subtitle">
            OTP verification error escalated with an inline warning banner counting down remaining attempts before a 30-min lockout. The 6 boxes still render in error state via status error and inline caption; warning banner sits below using Ux4gStatusBanner.
          </p>
        </div>
      </div>
            ...
            {activeMainTab === 'code' && (
              <div className="wb-code-area">
                ...
                <CodeBlock
                  code={codeString}
                  language="TSX"
                  filename={variant === 'card' ? 'AuthOtpAttemptWarningCardPattern.tsx' : 'AuthOtpAttemptWarningDefaultPattern.tsx'}
                />
              </div>
            )}
          </div>
...
export default AuthAttemptWarningDoc;
