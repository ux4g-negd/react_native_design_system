...
interface PaymentFailedDocProps {
  isDark: boolean;
}
...
export const PaymentFailedDoc: React.FC<PaymentFailedDocProps> = ({ isDark }) => {
  ...
  const colors = useMemo(() => {
    ...
    return {
      ...
      subtleText: isDark ? UX4GColors.neutral400 : UX4GColors.neutral700,
      primary: UX4GColors.primary,
      primaryLight: UX4GColors.primary300,
      // Error red palette tokens from Flutter
      redOuterBg: isDark ? UX4GColors.red800 : UX4GColors.red100,
      redIconColor: isDark ? UX4GColors.red500 : UX4GColors.red600,
      redBorder: isDark ? UX4GColors.red600 : UX4GColors.red300,
    };
  }, [isDark, variant]);

  // Clean React Native TSX code snippet matching Flutter paymentFailedComponent
...
export const PaymentFailedCardPattern = ({ isDark }: { isDark: boolean }) => {
  const details = [
    { label: 'Reason', value: 'Bank declined' },
    { label: 'Attempted amount', value: 'Rs 41.30' },
  ...
  return (
    <View style={[styles.screen, { backgroundColor: isDark ? UX4GColors.primary800 : UX4GColors.primary100 }]}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        {/* Floating White Card Container (Wraps Stepper + Red Error Badge + Details) */}
        <View style={[styles.card, { backgroundColor: isDark ? UX4GColors.neutral900 : UX4GColors.neutral0 }]}>
          ...
          {/* Red Error Filled Circle Badge (Icons.error in Flutter) */}
          <View style={[styles.outerBadge, { backgroundColor: isDark ? UX4GColors.red800 : UX4GColors.red100 }]}>
            <View style={[styles.filledErrorCircle, { backgroundColor: isDark ? UX4GColors.red500 : UX4GColors.red600 }]}>
          ...
          {/* Subtitle */}
          <Text style={[styles.subtitle, { color: isDark ? UX4GColors.neutral200 : UX4GColors.neutral700 }]}>
            Bank declined the transaction. No amount was deducted from your account.
          </Text>
          ...
          <View
            style={[
              styles.detailBox,
              {
                backgroundColor: isDark ? UX4GColors.red800 : UX4GColors.red100,
          ...
          >
            {details.map((row, idx) => (
              <View key={idx} style={styles.detailRow}>
                ...
                <Text style={[styles.detailValue, { color: isDark ? UX4GColors.neutral50 : UX4GColors.neutral900 }]}>
                  {row.value}
                </Text>
              </View>
            ))}
        ...
        <Ux4gButton text="Try again with UPI" variant="primary" size="large" style={{ marginTop: 20 }} />

        <Ux4gButton text="Try a different method" variant="outline" size="large" style={{ marginTop: 10 }} />
...
const styles = StyleSheet.create({
  ...
  filledErrorCircle: { width: 32, height: 32, borderRadius: 16, alignItems: 'center', justifyContent: 'center' },
  exclamationText: { color: '#FFFFFF', fontSize: 20, fontWeight: '800' },
...
const styles = StyleSheet.create({
  ...
  // Live interactive mockup matching Flutter paymentFailedComponent (_PaymentFailedMockup / _PaymentFailedCardMockup) EXACTLY
  const renderLiveMockup = () => {
    ...
    return (
      ...
      >
        ...
        >
          {/* Card Container (Wraps Stepper + Red Error Badge + Details) */}
          <div
            style={{
          ...
          >
            ...
            {/* Red Error Filled Circle Badge (Exact Flutter Icons.error 32px filled red circle) */}
            <div
              style={{
                width: 64,
                height: 64,
                borderRadius: '50%',
                backgroundColor: colors.redOuterBg,
                display: 'flex',
                alignItems: 'center',
                justifyContent: 'center',
            ...
            >
              <svg width="32" height="32" viewBox="0 0 32 32">
                {/* Filled Solid Red Circle matching Flutter Icons.error */}
                <circle cx="16" cy="16" r="16" fill={colors.redIconColor} />
                {/* White Exclamation mark inside */}
                <rect x="14.5" y="7" width="3" height="11" rx="1.5" fill="#FFFFFF" />
                <circle cx="16" cy="22.5" r="1.75" fill="#FFFFFF" />
              </svg>
            </div>
          ...
          <div style={{ marginTop: 20, display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%' }}>
            {/* Try Again Solid Primary Button */}
            <button
              type="button"
              onClick={() => alert('Retrying with UPI...')}
              style={{
                width: '100%',
                height: 48,
            ...
            {/* Try a different method Outline Button */}
            <button
              type="button"
              onClick={() => alert('Choosing different method...')}
              style={{
                marginTop: 10,
                width: '100%',
                height: 48,
            ...
            {/* Pay at CSC centre Ghost Button */}
            <div style={{ textAlign: 'center', marginTop: 12 }}>
              <button
                type="button"
                onClick={() => alert('Pay at CSC centre')}
                style={{
    ...
    <div className="wb-page">
      ...
      <div className="wb-header">
        <div className="wb-header-row">
          <h1 className="wb-title">Payment Failed</h1>
          <span className="wb-badge">Pattern</span>
        </div>
        <p className="wb-subtitle">
          Error screen when a payment is declined. Displays a red badge, a red-tinted details table with the failure reason, and retry/alternative-method CTAs. Toggle between flat phone layout and card-style layout. Mobile-sized layout (360px).
        </p>
      </div>
